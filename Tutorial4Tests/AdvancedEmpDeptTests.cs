...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldReturnEmployeeManagerPairs()
    {
        ...
        var result = emps.Join(emps, ee => ee.Mgr, m => m.EmpNo,
            (ee, m) => new { Employee = ee.EName, Manager = m.EName }).ToList();
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var result = emps
            .Join(depts, e => e.DeptNo, d => d.DeptNo, (e, d) => new { e, d }).SelectMany(r => grades.Where(s => r.e.Sal >= s.Losal && r.e.Sal <= s.Hisal),
                (ed, s) => new { EName = ed.e.EName, DName = ed.d.DName, Grade = s.Grade }).ToList();
